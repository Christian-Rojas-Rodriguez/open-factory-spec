...
1. Invoke `architect` to orchestrate.
2. `architect` reads the current `workflow.md` and the open Tasks under `.claude/specs/tasks/`.
3. `architect` invokes `researcher` if external context has changed (new stack piece, new domain dependency).
4. `architect` invokes `planner` to produce a **diff proposal** — what to add, modify, or remove in the Workflow.
